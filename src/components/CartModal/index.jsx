...
import {
    decCount,
    incCount, modifierClear,
    productClear, productSum, removeProductFromCart, saveChangeInCart,
    toggleModal
            ...
            <div className={`product-modal__container ${modalActive ? "product-modal__container-active" : ""}`}>
                ...
                <div className="product-modal__action">
                    <div className="product__count">
                        <span onClick={() => product.count > 0 && decrementCount()}>-</span>
...

                    {
                        product.count > 0
                            ?  <button
                                ...
                                onClick={saveChange}>Сахранить <span>{product.count * (product.price.amd + sum)} ֏</span>
                            </button>
                            :<button
                                className="btn btn-red btn-remove"
                                onClick={() => removeFromCart(product.cartId)}>Удалить <span>{product.count * (product.price.amd + sum)} ֏</span>
                            </button>
                    }
